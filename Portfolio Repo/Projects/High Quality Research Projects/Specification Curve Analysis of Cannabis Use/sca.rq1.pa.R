...
#
resultsframe.rq1.pa <- function(input) {
  ### SETUP SPECIFICATION NAMES
  levels.outcome <- input
  levels.affect.variable <- c("mood.positive", "mood.cheerful", "mood.friendly", "mood.calm", "mood.happy", "mood.engaged")
...
curve.rq1.pa <- function(input) {
  ...
  for (n in 1:nrow(results.frame)) {
    ...
    if (results.frame$affect.variable[n] == "mood.positive") {
      data <- data %>%
        drop_na(mar.binary, mood.positive)
    }
    else if (results.frame$affect.variable[n] == "mood.cheerful") {
      data <- data %>%
        drop_na(mar.binary, mood.cheerful)
    }
    else if (results.frame$affect.variable[n] == "mood.friendly") {
      data <- data %>%
        drop_na(mar.binary, mood.friendly)
    }
    else if (results.frame$affect.variable[n] == "mood.calm") {
      data <- data %>%
        drop_na(mar.binary, mood.calm)
    }
    else if (results.frame$affect.variable[n] == "mood.happy") {
      data <- data %>%
        drop_na(mar.binary, mood.happy)
    }
    else if (results.frame$affect.variable[n] == "mood.engaged") {
      data <- data %>%
        drop_na(mar.binary, mood.engaged)
    }
    ...
    else if (results.frame$control.variable[n] == "age") {
      model <- glmer(mar.binary ~ data[,affectvar] + age + (1 | PID), family = binomial, data = data)
      nullmodel <- glmer(mar.binary ~ age + (1 | PID), family = binomial, data = data)
      results.frame$effect[n] <- summary(model)$coefficients[2,1]
      results.frame$p.value[n] <- anova(model, nullmodel)[2,"Pr(>Chisq)"]        
    }   
  }
...
# run SCA
#
results.frame.rq1.pa <- resultsframe.rq1.pa(c("mar.binary"))
results.rq1.pa <- curve.rq1.pa(results.frame.rq1.pa)
#
sum(results.rq1.pa$p.value < .05)/length(results.rq1.pa$p.value) # % of significant specifications
median(exp(results.rq1.pa$effect)) # median effect size
#
##################################################
#
# 4) permutation test
...
# setup empty frame
#
resultsframe.shuffle.rq1.pa <- function(input) {
  ### SETUP SPECIFICATION NAMES
  levels.outcome <- input
  levels.affect.variable <- c("mood.positive", "mood.cheerful", "mood.friendly", "mood.calm", "mood.happy", "mood.engaged")
  levels.inclusion.criterion <- c("prior.use.any", "prior.use.30.days", "prior.use.30.days.high")
...
# fill empty frame with results
#
curve.shuffle.rq1.pa <- function(input) {
  results.frame <- input
  affectvar <- NA
  model <- NA
  nullmodel <- NA
...
#
results.frame.shuffle.rq1.pa <- resultsframe.shuffle.rq1.pa(c("mar.binary"))
results.curve.shuffle.rq1.pa <- curve.shuffle.rq1.pa(results.frame.shuffle.rq1.pa)
#
#
...
#
results.curve.shuffle.rq1.pa$shuffle <- as.factor(as.character(results.curve.shuffle.rq1.pa$shuffle))
sum(extract_fraction(results.rq1.pa)<=extract_threshold(results.curve.shuffle.rq1.pa)) # count number of shuffled datasets with more sig specs
mean(extract_fraction(results.rq1.pa)<=extract_threshold(results.curve.shuffle.rq1.pa)) # calculate p-value of permutation test
#
save.image('rq1.pa.RData', compress = "xz")
#
##################################################
#
# 5) plots
...
scatter.frame <- function(results.frame) {
  ...
  results.frame.long$label <- ordered(results.frame.long$label,
                                      levels = c("median", "averaged2", 
                                                 ...
                                                 "mood.engaged", "mood.happy", "mood.friendly", "mood.calm", "mood.cheerful", "mood.positive")) 
  library(plyr)
  results.frame.long$label <- revalue(results.frame.long$label,
                                      c("mood.positive" = "Positive affect: average", "mood.cheerful" = "Positive affect: cheerful", "mood.calm" = "Positive affect: calm", "mood.friendly" = "Positive affect: friendly", "mood.happy" = "Positive affect: happy", "mood.engaged" = "Positive affect: engaged",
...
#ggsave(filename = "rq1.pa.png", dpi = 600, height = 12, width = 18)
...
#
ggpubr::ggarrange(temp.plot, temp.plot2, nrow = 2, ncol = 1)
#ggsave(filename = "rq1.pa.full.png", dpi = 600, height = 12, width = 15)
